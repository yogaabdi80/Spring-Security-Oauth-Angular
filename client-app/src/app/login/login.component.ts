...
export class LoginComponent implements OnInit {
  ...
  login() {
    this.authService.login(this.username,this.password).subscribe(data => {
      if (data.status == 200) {
        window.location.href=`http://localhost:8081/auth/server/oauth/authorize?client_id=${this.client_id}&response_type=${this.response_type}&redirect_uri=${this.redirect_uri}`
      } else{
        alert(data.msg)
      }
    });
